...
def loadMesh(node,_filename,**kwargs):
    ...
    if splitedName[-1] in ['vtk','obj','stl','msh']:
        if splitedName[-1] == "msh":
            node.addObject("MeshGmshLoader", name="meshLoader",filename=_filename, **kwargs)
        else:
            node.addObject("Mesh"+splitedName[-1].upper()+"Loader", name="meshLoader",filename=_filename, **kwargs)
    else:
